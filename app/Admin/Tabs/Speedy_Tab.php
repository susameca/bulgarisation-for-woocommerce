	...
	public function add_profile_data_fields( $fields ) {
		if ( $this->container[ Client::SPEEDY_PROFILE ]->is_valid_profile( true ) ) {
			...
			if ( !empty( $all_profiles ) ) {
				...
				$fields[ 'speedy' ][] = new Fields\TrueFalse_Field( 'administrative_fee', __( 'Administrative fee on price calculations', 'bulgarisation-for-woocommerce' ), null, null, __( 'Usage of administrative fee is enabled and configured in client contract. Ignored if not applicable for user.', 'bulgarisation-for-woocommerce' ) );
