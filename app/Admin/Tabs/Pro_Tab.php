	...
	public function render_tab_html() {
		...
		<div class="woo-bg-text-wrapper">
			...
			if ( woo_bg_is_pro_activated() ) {
				...
				echo wp_kses_post( wpautop( __( 'There is a Pro version of the plugin as an addon ( additional plugin to the free one ).', 'bulgarisation-for-woocommerce' ) ) );
			}
			
			echo wp_kses_post( wpautop( __( 'This version of the plugin adds a couple of checkout functionalities:', 'bulgarisation-for-woocommerce' ) ) );
			?>
			<ul>
				<li>
				...
				<li>
					<?php echo wp_kses_post( wpautop( " - " .  __( 'Remove the limit of 100 orders for NRA XML Export for N-18', 'bulgarisation-for-woocommerce' ) ) ); ?>
				</li>
			</ul>
			<?php
