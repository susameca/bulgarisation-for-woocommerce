<template>
	<div class="woo-bg--cvc-delivery">
	  ...
	  <input 
	  	...
	  	v-if="( selectedAddress.type === 'streets' )"
	  ...
	  <input 
	  	...
	  	v-if="( selectedAddress.type === 'quarters' )"
	  	@keyup="streetNumberChanged"
	  >
	</div>
</template>
	...
	methods: {
		...
		checkFields() {
			...
			this.countryField.on('change', function () {
				setCookie( 'woo-bg--cvc-address', '', 1 );
				_this.state = $(this).val();
				_this.loadCity();
			});
		},
		loadLocalStorage(){
			let localStorageData = localStorage.getItem( 'woo-bg--cvc-address' );
